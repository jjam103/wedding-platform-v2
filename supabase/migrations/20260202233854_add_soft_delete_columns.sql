-- Migration: Add Soft Delete Support (Modified - without admin_users references)
...
CREATE POLICY "Guests can view published content pages" ON content_pages FOR SELECT USING (status = 'published' AND deleted_at IS NULL);

DROP POLICY IF EXISTS "Guests can view sections" ON sections;
CREATE POLICY "Guests can view sections" ON sections FOR SELECT USING (deleted_at IS NULL);

DROP POLICY IF EXISTS "Guests can view columns" ON columns;
CREATE POLICY "Guests can view columns" ON columns FOR SELECT USING (deleted_at IS NULL);

DROP POLICY IF EXISTS "Guests can view events" ON events;
CREATE POLICY "Guests can view events" ON events FOR SELECT USING (deleted_at IS NULL);
...
CREATE POLICY "Guests can view approved photos" ON photos FOR SELECT USING (moderation_status = 'approved' AND deleted_at IS NULL);
